...
media_preco = (
    df_filtrado.groupby("Marca")["Preço Atual (R$)"]
    .mean()
...
media_satisfacao = (
    df_filtrado.groupby("Marca")["Nota de Avaliação"]
    .mean()
    .sort_values(ascending=False)
    .reset_index()
)
...
df_valid = df_filtrado[
    ...
    & (df_filtrado["Qtd. de Avaliações"] > 0)
]

# Fórmula revisada para considerar quantidade de avaliações
df_valid["Score"] = (
    df_valid["Nota de Avaliação"] * df_valid["Qtd. de Avaliações"]
